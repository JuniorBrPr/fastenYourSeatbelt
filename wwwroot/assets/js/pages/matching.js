...
const userInterestsCount = await getBuddyInterests(FYSCloud.Session.get("userId")).then(
	(data) => data.length
);
...
async function populateList(buddyList, type, data) {
	...
	if (data.length === 0) {
		...
		} else if (userInterestsCount === 0) {
			emptyListMsg.setAttribute("data-translate", "match.noInterests");
		} else {
